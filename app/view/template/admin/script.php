<!-- Bootstrap core JavaScript-->
<script src="<?= BASE_URL; ?>themes/vendor/jquery/jquery.min.js"></script>
<script src="<?= BASE_URL; ?>themes/vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

<!-- Core plugin JavaScript-->
<script src="<?= BASE_URL; ?>themes/vendor/jquery-easing/jquery.easing.min.js"></script>

<!-- Custom scripts for all pages-->
<script src="<?= BASE_URL; ?>themes/js/sb-admin-2.min.js"></script>

<!-- Page level plugins -->
<script src="<?= BASE_URL; ?>themes/vendor/chart.js/Chart.min.js"></script>

<!-- Page level custom scripts -->
<script src="<?= BASE_URL; ?>themes/js/demo/chart-area-demo.js"></script>
<script src="<?= BASE_URL; ?>themes/js/demo/chart-pie-demo.js"></script>

<!-- Page level plugins -->
<script src="<?= BASE_URL; ?>themes/vendor/datatables/jquery.dataTables.min.js"></script>
<script src="<?= BASE_URL; ?>themes/vendor/datatables/dataTables.bootstrap4.min.js"></script>

<!-- Page level custom scripts -->
<script src="<?= BASE_URL; ?>themes/js/demo/datatables-demo.js"></script>